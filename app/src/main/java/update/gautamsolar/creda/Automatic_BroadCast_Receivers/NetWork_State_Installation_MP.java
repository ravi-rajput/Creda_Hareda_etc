...
public class NetWork_State_Installation_MP extends BroadcastReceiver {
    ...
    public void onReceive(Context context, Intent intent) {
        ...
        List<Installation_Table_MP> credaModels = getAlldate();
...
//            Foundation_Table_MP foundation_table_mp = credaModels.get(i);
            Installation_Table_MP installation_table_mp= credaModels.get(i);
            loadNames(installation_table_mp.foto1, installation_table_mp.foto2, installation_table_mp.foto3, installation_table_mp.foto4,installation_table_mp.eng_id, installation_table_mp.Lat, installation_table_mp.Lon, installation_table_mp.Regn
                    , installation_table_mp.Dati, installation_table_mp.rmu_status);
            Log.d("DATACONTACT", String.valueOf(installation_table_mp.Eng_contact));
    ...
    public void loadNames(final String phot1, final String phot2, final String phot3, final String phot4, final String eng_id, final String Latti, final String Longi,
                          final String RegisterNo, final String Date_time,final String rmu_status) {
...
try{
                                new Delete().from(Installation_Table_MP.class).where("Regn =" + RegisterNo).execute();
                                Toast.makeText(context, "Installation MP Data Upload Successfully", Toast.LENGTH_SHORT).show();
}catch (Exception ae){
}
                            }
                        } catch (JSONException e) {
                            e.printStackTrace();
                            Toast.makeText(context, "Installation MP Data Upload failed", Toast.LENGTH_LONG).show();
                        }
                    }
                },
            ...
            protected Map<String, String> getParams() throws AuthFailureError {
               ...
               params.put("rmu_status", rmu_status);
                return params;
            }
        };
    ...
    public List<Installation_Table_MP> getAlldate() {
        //Getting all items stored in Inventory table
        return new Select()
                .from(Installation_Table_MP.class)
                .execute();
    }

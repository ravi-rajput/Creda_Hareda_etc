...
public class NetworkstateCheckerRMuNEw extends BroadcastReceiver {
    ...
    public void onReceive(Context context, Intent intent) {
        ...
        List<RMU_Table> rmu_upload_table = getAlldate();
        //Adding all the items of the inventories to arraylist
        for (int i = 0; i < rmu_upload_table.size(); i++) {
            RMU_Table rmu = rmu_upload_table.get(i);
            loadNames(rmu.fotoRMu, rmu.latrmu, rmu.lonrmu, rmu.datetime, rmu.NewRMuNO, rmu.eng_id, rmu.regn_no_rmu);
        ...
        StringRequest stringRequest = new StringRequest(Request.Method.POST, constants.RMU_API,
                    ...
                    public void onResponse(String response) {
                        ...
                        try {
                            ...
                            if (upload.equals("false")) {
                                ...
                                try{
                                new Delete().from(RMU_Table.class).where("regn_no_rmu =" + Regn_No).execute();
                                Toast.makeText(context, "RMU Data Creda Upload Successfully", Toast.LENGTH_SHORT).show();
                                }catch (Exception ae){
                                }
                            }
                        } catch (JSONException e) {
                            Toast.makeText(context, "RMU Data Creda Upload failed", Toast.LENGTH_LONG).show();
                            e.printStackTrace();
                        }
                    }
                },
                new Response.ErrorListener() {
                    ...
                    public void onErrorResponse(VolleyError error) {
                        Toast.makeText(context, "RMU Data Upload Creda failed", Toast.LENGTH_LONG).show();
    ...
    public List<RMU_Table> getAlldate() {
        //Getting all items stored in Inventory table
        return new Select()
                .from(RMU_Table.class)
                .execute();
    }

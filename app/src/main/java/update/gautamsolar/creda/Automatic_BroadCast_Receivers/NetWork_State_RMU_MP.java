...
public class NetWork_State_RMU_MP extends BroadcastReceiver {
    ...
    public void onReceive(Context context, Intent intent) {
        ...
        List<Rmu_Table_MP> rmu_table_mps = getAlldate();
        //Adding all the items of the inventories to arraylist
        for (int i = 0; i < rmu_table_mps.size(); i++) {
            Rmu_Table_MP rmu_table_mp = rmu_table_mps.get(i);
            loadNames(rmu_table_mp.fotoRMu, rmu_table_mp.latrmu, rmu_table_mp.lonrmu, rmu_table_mp.datetime, rmu_table_mp.NewRMuNO, rmu_table_mp.eng_id, rmu_table_mp.regn_no_rmu);
...
try{
                                new Delete().from(Rmu_Table_MP.class).where("regn_no_rmu =" + Regn_No).execute();
                                Toast.makeText(context, "RMU Data MP Upload Successfully", Toast.LENGTH_SHORT).show();
}catch (Exception ae){
}
                            }
                        } catch (JSONException e) {
                            Toast.makeText(context, "RMU Data MP Upload failed", Toast.LENGTH_LONG).show();
                            e.printStackTrace();
                        }
                    }
                },
    ...
    public List<Rmu_Table_MP> getAlldate() {
        //Getting all items stored in Inventory table
        return new Select()
                .from(Rmu_Table_MP.class)
                .execute();
    }
